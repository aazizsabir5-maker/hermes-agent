...
- Known limitations: The structural validator does not judge semantic design quality or evidence truth; completion/applicability detection remains heuristic; universal gateway, cron, ACP, proxy, media, subagent, and tamper-resistant release enforcement are outside scope; the full repository suite is not a pass—84 failures were reproduced on unmodified `upstream/main`, and no changed-surface failure was identified.
