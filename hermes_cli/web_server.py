...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The enumeration is LIVE (see _LiveProfileHomes) rather than a snapshot
    taken at startup, because desktop bots are profiles the app mints while it
    runs: onboarding creates the setup bot and then a task bot mid-session,
    and each schedules its own check-ins into a HERMES_HOME that did not exist
    when a startup list would have been captured.
    ...
    if isinstance(provider, InProcessCronScheduler):
        # _LiveProfileHomes fails open to the active profile on its own, so
        # the ticker still runs if enumeration breaks.
        start_kwargs["profile_homes"] = _LiveProfileHomes()
