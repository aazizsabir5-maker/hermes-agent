...
def finalize_turn(
    ...
    *,
    ...
    # Apply only the policies scoped at turn start. This gate replaces an
    # unsupported completion claim before the current assistant row is
    # persisted; it creates no receipt, snapshot, hash, or audit state.
    _finalization_result = None
    ...
    if final_response and not interrupted and active_policy_ids:
        try:
            ...
            context = FinalizationContext.for_response(
                ...
                metadata={"active_policy_ids": tuple(active_policy_ids)},
            )
            _finalization_result = apply_finalization_gate(context, policies)
            final_response = _finalization_result.response_text
            if not _finalization_result.allowed:
                completed = False
                failed = True
    ...
    buffered_callbacks = getattr(agent, "_decision_buffered_callbacks", None)
    if buffered_callbacks:
        (
            agent.stream_delta_callback,
            agent.interim_assistant_callback,
            agent.reasoning_callback,
        ) = buffered_callbacks
        agent._decision_buffered_callbacks = None
